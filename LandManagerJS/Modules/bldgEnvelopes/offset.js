...
function setCustomSetbacks() {
    var ogParcel = map.getLayer('originalParcel');
    if (ogParcel) {
        var setbacks = ogParcel.graphics[0] ? ogParcel.graphics[0].attributes : {};
        ...
        var form = new dijit.form.Form({
            ...
            onSubmit: function (event) {
                ...
                ogParcel.graphics[0].attributes.Front = f.value;
                ogParcel.graphics[0].attributes.Side = s.value;
                ogParcel.graphics[0].attributes.Rear = r.value;

                document.getElementById('beInfoMeasureContainer').innerHTML = "Differnt from code<hr/>Front: " + f.value + "<br/>Rear: " + r.value + "<br/>Side: " + s.value;
                dlg.destroy();
            }
        });
        var dlg = new dijit.Dialog({
            content: form,
            "title": "Change Setback Values",
            "style": "width: 300px;"
        }).placeAt(dojo.body());
        
        var f = new dijit.form.NumberSpinner({
            value: setbacks.Front,
            placeHolder: "Front",
            style: "width:50%;",
            onChange: function (val) {
                if (f.value < 1 || r.value < 1 || s.value < 1) {
        ...
        form.domNode.appendChild(dojo.doc.createTextNode(" Front "));

        var r = new dijit.form.NumberSpinner({
            value: setbacks.Rear,
            placeHolder: "Rear",
            style: "width:50%;",
            onChange: function (val) {
                if (s.value < 1 || r.value < 1 || f.value < 1) {
                    okBtn.set('disabled', true);
                } else {
                    okBtn.set('disabled', false);
                }
        ...
        form.domNode.appendChild(dojo.doc.createTextNode(" Rear "));

        var s = new dijit.form.NumberSpinner({
            value: setbacks.Side,
            placeHolder: "Side",
            style: "width:50%;",
            onChange: function (val) {
                if (r.value < 1 || s.value < 1 || f.value < 1) {
                    okBtn.set('disabled', true);
                } else {
                    okBtn.set('disabled', false);
                }
        ...
        form.domNode.appendChild(dojo.doc.createTextNode(" Side "));
