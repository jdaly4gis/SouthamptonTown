...
function dynamicMeasurements(sender) {
    ...
    hoverHandler = map.on("mouse-move", function (evt) {
        if (measureToolbar._graphic) {
            ...
            var adjacent = startPoint[0] - endPoint.x;
            var opposite = startPoint[1] - endPoint.y;
...
function DMS(val) {
    ...
    m = ms[0];
    s = ("." + ms[1]) * 60;
    return d + "&deg " + m + "' " + Math.round(s * 10000) / 10000 + "&quot";
}
